...
// Saved cost (qənaət) only means something once BOTH the planned budget
// (the initial vendor quote) and used_budget (the real final spend, set
// later) are known for a row — a row where either is still null/0 is
// simply excluded, not treated as 0 saved, so an incomplete plan doesn't
// silently drag the total toward "fully saved".
export function hasSavedCost(t) {
  return !!t.budget && !!t.used_budget;
}
