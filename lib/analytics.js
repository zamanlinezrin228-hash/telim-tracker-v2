...
// Saved cost (qənaət) only means something once a row has actually finished:
// - status must be 'Completed' — a Cancelled/Postponed/Scheduled/In Progress
//   row hasn't produced a real final spend yet, so counting its unspent
//   planned budget as "saved" would overstate savings for training that was
//   simply never delivered (or not delivered yet), not efficiently delivered.
// - both budget (planned) and used_budget (real final spend) must be set —
//   checked with `!= null` rather than truthy, since used_budget = 0 is a
//   valid, common real value (a training delivered internally at no cost)
//   and must still count as full savings, not get excluded like a missing
//   value would.
export function hasSavedCost(t) {
  return t.status === 'Completed' && t.budget != null && t.used_budget != null;
}
...
export function computeKPIs(trainings) {
  ...
  const totalUsedBudget = trainings.reduce((a, t) => a + (Number(t.used_budget) || 0), 0);
  ...
  return {
    ...
    totalBudget, totalUsedBudget, totalSavedCost, totalHours,
    ...
    // Actual spend as a share of planned budget — only meaningful once
    // used_budget starts getting filled in (AddToPlanModal/TrackingView),
    // so 0 planned budget just reads as 0% rather than dividing by zero.
    budgetUtilization: totalBudget ? Math.round((totalUsedBudget / totalBudget) * 100) : 0,
    atRiskCount,
  };
}
