...
import { countUnseenRequests, normalizeName, isAnnualTnaRequest } from '../lib/helpers';
...
export default function Home() {
  ...
  // Only the requests list — used by the lightweight notification refresh
  // below, so a status change elsewhere shows up without reloading trainings.
  const loadRequests = useCallback(async () => {
    const { data: rData, error } = await sb.from('training_requests').select('*').order('created_at', { ascending: false });
    if (!error) setRequests(rData || []);
  }, []);
  ...
  // Live notifications for EVERY user (not only L&D): every 30 s, and
  // immediately when the browser tab regains focus, fetch a tiny
  // "signature" of the requests visible to this user (row count + newest
  ...
  // On ENTERING İllik TNA, remember the previous "last seen" so the hub's
  // tabs can highlight what's new during this visit.
  useEffect(() => {
    if (!loggedIn || !profile || view !== 'annual-tna') return;
    setTnaUnseenSince(tnaLastSeen || null);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [view, loggedIn, profile?.id]);

  // While the user is ON "Təlim Sorğuları" / "İllik TNA", keep everything
  // currently loaded marked as seen — re-run whenever the requests list
  // changes (after the user's own approve/reject/edit, or a live refresh),
  // so their own actions never bring the red badge back.
  // The stamp is max(now, newest created_at/updated_at of the loaded rows)
  // + 1 ms rather than just the browser clock, so a PC clock that's a bit
  // behind the server can't leave already-seen rows counted as "new".
  // Writes to the DB only when the stamp actually moves forward.
  useEffect(() => {
    if (!loggedIn || !profile || (view !== 'requests' && view !== 'annual-tna')) return;
    const kind = view === 'annual-tna' ? 'tna' : 'adhoc';
    const field = view === 'annual-tna' && hasTnaSeenColumn ? 'last_seen_tna_at' : 'last_seen_requests_at';
    let maxTs = Date.now();
    requests.forEach((r) => {
      if (hasTnaSeenColumn && (kind === 'tna') !== isAnnualTnaRequest(r)) return;
      const c = new Date(r.created_at).getTime();
      const u = new Date(r.updated_at).getTime();
      if (c > maxTs) maxTs = c;
      if (u > maxTs) maxTs = u;
    });
    const current = profile[field] ? new Date(profile[field]).getTime() : 0;
    if (current > maxTs) return;
    const stamp = new Date(maxTs + 1).toISOString();
    setProfile((p) => (p ? { ...p, [field]: stamp } : p));
    sb.from('profiles').update({ [field]: stamp }).eq('id', profile.id).then(({ error }) => {
      if (error) console.warn('Bildiriş "baxıldı" vaxtı yazılmadı:', error.message);
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [view, loggedIn, profile?.id, requests]);
