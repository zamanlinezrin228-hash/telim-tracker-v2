...
export default function DashboardView({ trainings }) {
  ...
  const statusCounts = {};
  trainings.forEach(t => { statusCounts[t.status] = (statusCounts[t.status] || 0) + 1; });
  const statusRows = Object.entries(statusCounts).sort((a, b) => b[1] - a[1]);
  const maxStatus = Math.max(...statusRows.map(r => r[1]), 1);

  const deptCounts = {};
  trainings.forEach(t => { deptCounts[t.dept] = (deptCounts[t.dept] || 0) + 1; });
  ...
  return (
    <div>
      <div className="hero">
        ...
        <p>Mars Overseas — Təlim Ehtiyacları (TNA) məlumatları əsasında canlı izləmə və analiz.</p>
      </div>
      <div className="page">
