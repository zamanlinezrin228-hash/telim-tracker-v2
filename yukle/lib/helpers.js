...
// A manager's scope is their WHOLE reporting tree (every level below them,
// resolved server-side by get_my_scope_profiles() and stored on the profile
// as scope_name_set in pages/index.js), not just direct reports — plus the
// older dept/şöbə string match as a fallback for rows whose submitter isn't
// linked into the manager_id chain.
export function matchesOwnScope(row, profile) {
  if (!row || !profile) return false;
  const set = profile.scope_name_set;
  if (set && set.size && set.has(normalizeName(row.employee_name))) return true;
...
export function isAnnualTnaRequest(r) {
  return r.source === 'Manager Survey';
}

// Ad-hoc (Təlim Sorğuları) and annual-TNA (İllik TNA) requests each have
// their own "last seen" timestamp, so visiting one page never clears the
// other's red badge. `kind`: 'adhoc' | 'tna' | undefined (both).
export function countUnseenRequests(requests, profile, scopeFlags, kind, lastSeenAt) {
  if (!profile) return 0;
  const seenAt = lastSeenAt !== undefined ? lastSeenAt : profile.last_seen_requests_at;
  return requests.filter((r) => {
    if (kind === 'tna' && !isAnnualTnaRequest(r)) return false;
    if (kind === 'adhoc' && isAnnualTnaRequest(r)) return false;
    return inNotificationScope(r, profile, scopeFlags) && isUnseenRequest(r, seenAt);
  }).length;
}
